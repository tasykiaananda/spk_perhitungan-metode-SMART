...
const state = {
    ...
    penilaian: [
        ...
        { alt_id: 1, K1: 200, K2: 5, K3: 2, K4: 3, K5: 3 },
        { alt_id: 2, K1: 200, K2: 3, K3: 3, K4: 5, K5: 3 },
        { alt_id: 3, K1: 204, K2: 5, K3: 2, K4: 5, K5: 5 },
        { alt_id: 4, K1: 210, K2: 3, K3: 4, K4: 3, K5: 3 },
        { alt_id: 5, K1: 212, K2: 3, K3: 3, K4: 5, K5: 3 },
        { alt_id: 6, K1: 220, K2: 5, K3: 3, K4: 3, K5: 3 },
        { alt_id: 7, K1: 195, K2: 3, K3: 4, K4: 3, K5: 3 },
        { alt_id: 8, K1: 225, K2: 5, K3: 2, K4: 5, K5: 3 },
        { alt_id: 9, K1: 221, K2: 3, K3: 5, K4: 3, K5: 3 },
        { alt_id: 10, K1: 240, K2: 3, K3: 5, K4: 3, K5: 3 }
    ],
    hasil: {
        utilitas: [],
        terbobot: [],
...
const app = {
    ...
    getKriteriaBobot: function() {
        ...
        return state.kriteria.map(k => {
            ...
            let rawBobot = k.rating / totalRating;
    ...
    updateRating: function(index, value) {
        let val = parseInt(value);
        if(isNaN(val) || val <= 0) val = 1;
        state.kriteria[index].rating = val;
        this.renderKriteria();
        // Reset hasil since data changed
        document.getElementById('hasil-perhitungan').classList.add('hidden');
    },

    tambahKriteria: function() {
        ...
        state.kriteria.push({ id, nama, jenis, rating });
    ...
    tambahAlternatif: function() {
        ...
        const maxId = state.alternatif.length > 0 ? Math.max(...state.alternatif.map(a => a.id)) : 0;
        const newId = maxId + 1;

        state.alternatif.push({ id: newId, nama: nama });
    ...
    updatePenilaian: function(alt_id, kriteria_id, value) {
    ...
    hitungSMART: function() {
        ...
        const minMax = {};
        kriteria.forEach(k => {
            const values = state.penilaian.map(p => p[k.id] || 0);
            minMax[k.id] = {
                min: Math.min(...values),
                max: Math.max(...values)
            };
