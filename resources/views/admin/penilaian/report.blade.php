...
<head>
    ...
    <title>Laporan Matriks Penilaian - {{ date('d-m-Y') }}</title>
    <style>
        body {
...
<body>
    ...
    <div class="header">
        <h1>Laporan Matriks Penilaian Supplier</h1>
    ...
    <h2>Matriks Evaluasi Nilai Kriteria</h2>
    <table>
        <thead>
            <tr>
                <th>Nama Supplier</th>
                @foreach($kriterias as $k)
                    <th class="text-center">{{ $k->nama }} ({{ $k->id }})<br><small style="font-weight: normal; color: #666;">[{{ $k->jenis }}]</small></th>
                @endforeach
            </tr>
        </thead>
        <tbody>
                ...
                <tr>
                    ...
                    @foreach($kriterias as $k)
                        <td class="text-center">
                            {{ $matrix[$alt->id]['scores'][$k->id] ?? 0 }}
                        </td>
                    @endforeach
                </tr>
            @empty
                <tr>
                    <td colspan="{{ count($kriterias) + 1 }}" class="text-center">Belum ada data penilaian.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
